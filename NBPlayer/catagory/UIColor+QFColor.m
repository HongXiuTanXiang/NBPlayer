//
//  NBHomeViewController.m
//  NBPlayer
//
//  Created by yushang on 2019/7/21.
//  Copyright © 2019 Mac. All rights reserved.
//

#import "UIColor+QFColor.h"

@implementation UIColor (QFColor)

+(UIColor *) nbOringe{
    return [UIColor orangeColor];
}

+(UIColor *) nbWhite{
    return [UIColor whiteColor];
}

+(UIColor *) nbBlack{
    return [UIColor blackColor];
}

+(UIColor *) nbGray{
    return [UIColor colorWithHexString:@"#CCCCCC"];
}

+(UIColor*) nbDarkGray{
    return [UIColor colorWithHexString:@"#A9A9A9"];
}

+ (UIColor *)QFRandom {
    return [UIColor colorWithRed:arc4random_uniform(180.0)/ 255.0 green:arc4random_uniform(180.0)/ 255.0 blue:arc4random_uniform(180.0)/ 255.0 alpha:1.0];
}

// 16进制转color
+ (UIColor *)colorWithHexString:(NSString *)color
{
    NSString *cString = [[color stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceAndNewlineCharacterSet]] uppercaseString];
    
    // String should be 6 or 8 characters
    if ([cString length] < 6) {
        return [UIColor clearColor];
    }
    // 判断前缀
    if ([cString hasPrefix:@"0X"])
        cString = [cString substringFromIndex:2];
    if ([cString hasPrefix:@"#"])
        cString = [cString substringFromIndex:1];
    if ([cString length] != 6)
        return [UIColor clearColor];
    // 从六位数值中找到RGB对应的位数并转换
    NSRange range;
    range.location = 0;
    range.length = 2;
    //R、G、B
    NSString *rString = [cString substringWithRange:range];
    range.location = 2;
    NSString *gString = [cString substringWithRange:range];
    range.location = 4;
    NSString *bString = [cString substringWithRange:range];
    // Scan values
    unsigned int r, g, b;
    [[NSScanner scannerWithString:rString] scanHexInt:&r];
    [[NSScanner scannerWithString:gString] scanHexInt:&g];
    [[NSScanner scannerWithString:bString] scanHexInt:&b];
    
    return [UIColor colorWithRed:((float) r / 255.0f) green:((float) g / 255.0f) blue:((float) b / 255.0f) alpha:1.0f];
}


+ (UIColor *)colorWithHexString:(NSString *)hexSting alpha:(CGFloat)alpha {
    UIColor *color = [self colorWithHexString:hexSting];
    return [color colorWithAlphaComponent:alpha];
}

@end
